...
module.exports = function upload() {
  ...
  if (
    ...
    info.branchName.startsWith("greenkeeper-")
  ) {
    config.branchPrefix = "greenkeeper-";
  }
  ...
  const isInitial =
    info.branchName === config.branchPrefix + "initial" ||
    info.branchName === config.branchPrefix + "update-all";

  if (isInitial) {
    return console.error(
      "Not running on the initial Greenkeeper branch. Will only run on Greenkeeper branches that update a specific dependency",
    );
  }
  ...
  if (env.GH_TOKEN) {
    if (remote.slice(0, 5) !== "https")
      remote = `https://github.com/${info.repoSlug}`;
  ...
  console.info(
    `git remote add gk-origin ${remote} || git remote set-url gk-origin ${remote}`,
  );
  ...
  exec(
    `git push${
      process.env.GK_LOCK_COMMIT_AMEND ? " --force-with-lease" : ""
    } gk-origin HEAD:${info.branchName}`,
    {
      stdio: "inherit",
    },
  );
